 ...
 *
 * `0018_memory_retrieval.sql` is the argument for the keyword half and the
...
/** The FTS5 index. Created by `0018_memory_retrieval.sql`. */
...
export class MemoryStore {
   ...
   * `0018_memory_retrieval.sql` — so a `tier` other than `hot` returns nothing
  ...
  #requireKeywordIndex(): void {
    ...
    if (row === undefined) {
      throw new StoreError(
        "table_mismatch",
        `${KEYWORD_TABLE} does not exist. It is created by 0018_memory_retrieval.sql, so this ` +
