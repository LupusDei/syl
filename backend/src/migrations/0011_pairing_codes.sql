-- 0011_pairing_codes.sql — the short-lived secret a device is paired with.
--
-- Renumbered from 0009 during integration: two agents working in parallel
-- worktrees both claimed 0009, which the migration runner keys on. Safe to
-- move because nothing has ever run these against a real database — Syl is
-- not live yet. 0010_sync_log depends on 0009_todos_goals, so those two kept
-- their numbers and this one moved.
--
